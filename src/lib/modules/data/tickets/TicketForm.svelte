<script lang="ts">
  ...
  let form = $state({
    ...
    date: data?.date ?? new Date().toLocaleDateString("en-GB").replace(/\//g, "/"),
  });
...
>
  ...
  >
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
      <div>
        ...
        <p class="text-[12px] text-gray-400 mt-0.5">
          {mode === "add" ? "Fill in the details to raise a new ticket" : "Update the ticket information below"}
        </p>
      </div>
      <button
        onclick={onClose}
    ...
    <form class="px-6 py-5 flex flex-col gap-4 overflow-y-auto" onsubmit={handleSubmit}>
      ...
      <!-- Engineer -->
      <label class={labelClass}>
        <span class={labelTextClass}>Assigned Engineer <span class="text-red-400">*</span></span>
        <input
          type="text"
        ...
        />
        {#if errors.engineer}<span class={errorClass}>{errors.engineer}</span>{/if}
      </label>
