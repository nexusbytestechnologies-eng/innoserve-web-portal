...
<div class="flex min-h-screen bg-stone-100">
	...
	>
		...
		<div class="flex items-center gap-2.5 px-5 h-16 border-b border-white/10 shrink-0">
			...
			<div class="flex flex-col leading-none">
				<span class="text-[12px] font-bold tracking-[1px]">INNO<span class="text-[#E87D1F]">SERVE</span></span>
			...
			{#each navItems as item}
				...
				>
					...
					{:else if item.icon === 'hardhat'}
						<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 18a1 1 0 0 0 1 1h18a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v2z"/><path d="M10 10V5a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v5"/><path d="M4 15v-3a8 8 0 0 1 16 0v3"/></svg>
		...
		<div class="px-4 py-4 border-t border-white/10 shrink-0">
			<div class="flex items-center gap-3">
				<div class="w-8 h-8 rounded-full bg-[#E87D1F] flex items-center justify-center text-[11px] font-bold shrink-0">
					{initials}
				</div>
	...
	<div class="flex flex-col flex-1 lg:pl-60 min-w-0">
		...
		>
			...
			<div class="flex items-center gap-3">
				<span
					class="hidden sm:inline-flex px-3 py-1 rounded-full text-[11px] font-semibold tracking-[0.5px] {ROLE_BADGE_COLORS[role]}"
				>
					{ROLE_LABELS[role]}
				</span>
				...
				<div class="relative" data-notif>
					...
					>
						...
						{#if $unreadCount > 0}
							<span class="absolute top-1 right-1 w-4 h-4 rounded-full bg-[#E87D1F] text-white text-[9px] font-bold flex items-center justify-center leading-none">
					...
					{#if showNotifications}
						<div class="absolute top-11 right-0 w-80 bg-white border border-gray-200 rounded-[10px] shadow-[0_10px_30px_rgba(0,0,0,0.12)] z-50 overflow-hidden">
							<!-- Header -->
							<div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
								<span class="text-[13px] font-semibold text-[#0B182A]">Notifications</span>
								{#if $unreadCount > 0}
									<button
							...
							<div class="max-h-72 overflow-y-auto divide-y divide-gray-50">
								{#if $notifications.length === 0}
									<div class="px-4 py-8 text-center">
										...
										<p class="text-[12px] text-gray-400">No notifications yet</p>
									</div>
								{:else}
									{#each $notifications as n (n.id)}
										<div
											class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer {n.read ? 'opacity-60' : ''}"
											role="button"
											tabindex="0"
											onclick={() => { notifications.markRead(n.id); if (n.href) window.location.href = n.href; }}
											onkeydown={(e) => e.key === 'Enter' && notifications.markRead(n.id)}
										>
											<!-- Type dot -->
											...
											<div class="flex-1 min-w-0">
												<p class="text-[13px] font-medium text-[#0B182A] leading-snug">{n.title}</p>
												{#if n.message}
											...
											{#if !n.read}
												<div class="w-1.5 h-1.5 rounded-full bg-[#E87D1F] shrink-0 mt-1.5"></div>
											{/if}
										</div>
									{/each}
								{/if}
				...
				<div class="relative" data-dropdown>
					<button
						...
						class="w-9 h-9 rounded-full flex items-center justify-center text-white text-[13px] font-bold
